...
def main():
    ...
    for day in pd.date_range(start=START_DATE.strftime('%Y-%m-%d'), end=END_DATE.strftime('%Y-%m-%d'), freq='7D'):
        for district in districts:
            ...
            url = CALENDAR_BY_DIST_URL + str(district['state_id']) + '&district_id=' + str(
                district['district_id']) + '&date=' + day.strftime('%d-%m-%Y')
            logging.debug(url)
            response = requests.get(url)
            if (response.status_code == http.HTTPStatus.UNAUTHORIZED.value):
                logging.debug('Error saving state ' + str(district['state_id']) + ' and ' + str(
                    district['district_id']) + 'for date' + day.strftime('%d-%m-%Y'))
                continue
            else:
                centers = json.loads(response.text)['centers']
                summary['centers'] = len(centers)

                logging.info(str(len(centers)) + ' in ' + str(district['district_name']) + ' district , state - ' + str(
                    district['state_id']) + ' in upcoming 7 days from ' + day.strftime('%d-%m-%Y'))
                if(len(centers) > 0):
                    saveResponse(
                        'Centers_' + str(district['district_id']) + '_' + day.strftime('%d-%m-%Y'), centers)
                    available_centers = [c for c in centers if any(
                        s for s in c['sessions'] if s['min_age_limit'] != 45)]
                    paid_centers = [
                        c for c in centers if c["fee_type"] != "Free"]
                    summary['paid_centers'] = paid_centers
                    summary['centers_18'] = available_centers
                    if(len(paid_centers) > 0):
                        saveResponse(
                            'Paid_Centers_' + str(district['district_id']) + '_' + day.strftime('%d-%m-%Y'), paid_centers)
                    if(len(available_centers) > 0):
                        saveResponse('Available_18_Centers_' + str(
                            district['district_id']) + '_' + day.strftime('%d-%m-%Y'), available_centers)
